    ...
    override fun mouseClicked(mouseButtonEvent: MouseButtonEvent, bl: Boolean): Boolean {
        ...
        if (hoveredSlot === menu.slots.getOrNull(0)) {
            ...
            if (menu.isCraftingComplete) {
                if (button == 0) {
                    // 左键 → 取走成品
                    net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(WorkbenchCraftPacket(WorkbenchCraftPacket.Action.TAKE_RESULT))
                    return true
                }
            ...
            if (button == 0) {
                // 左键 → 开始合成
                net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(WorkbenchCraftPacket(WorkbenchCraftPacket.Action.START_CRAFT))
                return true
            } else if (button == 1) {
                // 右键 → 切换品质
                net.neoforged.neoforge.client.network.ClientPacketDistributor.sendToServer(WorkbenchCraftPacket(WorkbenchCraftPacket.Action.CYCLE_QUALITY))
                return true
            }
        }
        return super.mouseClicked(mouseButtonEvent, bl)
