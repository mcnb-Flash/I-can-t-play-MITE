...
open class BlockICPMWorkbench(
    ...
    override fun useWithoutItem(
        ...
        if (!level.isClientSide) {
            // NeoForge：BlockPos 经 IPlayerExtension.openMenu(MenuProvider, BlockPos) 下发客户端
            player.openMenu(getMenuProvider(state, level, pos), pos)
        }
        return InteractionResult.SUCCESS
    }

    open override fun getMenuProvider(state: BlockState, level: Level, pos: BlockPos): MenuProvider {
