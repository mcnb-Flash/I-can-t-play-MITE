...
- **修复：凝胶立方族从不自然生成**——Jelly / Blob / Ooze / Pudding 的刷怪函数此前"已定义、未挂接"，导致这一整族怪物在生存中根本刷不出。现挂接 `SpawnPlacements`（刷怪规则 `checkGelatinousCubeSpawnRules`：暗处 + 酸性条件）与群系刷怪（权重 8 / 6 / 6 / 4），双端同步。
- **修复：僵尸稀有掉落概率被砍到 1/5**——`ZombieMiteDropMixin` 旧用 `nextInt(base) != 0`（≈1/200），改回 R196 `nextInt(base) < 5 + 抢夺等级×2`（**标准 1/40、村民僵尸 1/10**，受抢夺加成）。

### P1 · 行为缺口（怪物特性不完整，已全闭）

- **新增：僵尸随机持械**——按 R196 补 `addRandomWeapon`：普通僵尸 **5%**、第 10 天起；村民僵尸 **20%**、第 20 天起；覆盖镐 / 斧 / 镰刀 / 草叉 / 丁字镐 / 短棒 / 木料（锈铁装备位暂以铜系替代，见下方"未闭"）。
- **新增：暗影 / 隐形追猎者熄灯**——3×3×（1+身高）范围内**火把→破坏、南瓜灯→还原为南瓜**；仅在距最近玩家 **> 4 格**时触发（贴身不熄灯）。
- **新增：暗影 / 夜翼命中致盲**——受其攻击命中后叠加视觉变暗（1.21 无 R196 对应效果，用 `DARKNESS` 近似）。
- **新增：骨领主 / 远古骨王召唤链**——按 R196 `r196BoneLordTick`：随从上限 **6**、召唤判定 `rand.nextInt(8) < 7 − 当前随从数`、含双召概率与射线/寻路校验；骨领主召普通骷髅、骨王召长逝。
- **新增：村民声望系统（整系统新建）**——R196 声望挂在"村庄"对象上，1.21.11 无村庄实体，故改为**按玩家全局累计**并随 Player NBT 持久化，钳制 **[−30, 10]**：
  - 交易 +1；被攻击 −1（未成年村民 −3）；被杀 −2；
  - 声望 ≤ **−15** 时，16 格内铁傀儡每 16 tick 扫描并将其设为攻击目标。

### P2 · 数值 / 机制偏差（已修 / 已核）

- **工具基准攻击伤害全量校准（本轮最大项）**——R196 语义是 `getBaseDamageVsEntity() + Material.getDamageVsEntity()`（玩家基础 1.0），而 1.21.11 的 `.sword(tier,x,spd)` / `XxxItem(tier,x,spd)` 最终修饰 = `x + tier.attackDamageBonus`；旧代码 x 比 R196 基准少 1.0，导致**整条武器线面板伤害低半颗心**。现把 `ICPMItems.kt` **72 处**注册参数全部改为 R196 基准（剑 4 / 匕首 2 / 小刀 1 / 斧 3 / 短斧 2 / 战斧 4 / 镐 2 / 战锤 2 / 锹 1 / 鸭嘴锄 1 / 锄 1 / 镰刀 1）；**原版工具**（木/石/铁/金/钻/下界合金）改不了注册参数，改由出口 mixin 补差值（对已校准的 ICPM 工具差值为 0，幂等安全）。
- **修复：黑曜石小刀材质误挂**——由 FLINT 改为 OBSIDIAN（耐久 400 → **800**）。
- **修复：木质短棍未纳入工具表**——`wood_cudgel` 补入 CUDGEL/WOOD 映射（攻击、耐久 200、击晕附魔、钝器特攻由此统一生效）。
- **近战乘区补全**（`calcRawMeleeDamageVs`）：
  - 银制武器 vs 亡灵：由"+2.5 平加（亡灵杀手近似）"改回 **×1.25 乘区**；
  - 短棍 / 战锤 vs 骷髅（含全部 ICPM 骷髅变种）：补 **×2.0**；
  - 悬液（水中 / 熔岩中）近战伤害：补 **`1 + (d−1)×0.5`** 减半；
  - **弱击**：未持工具（工具 ∪ 木棍 ∪ 骨头）且（生命 < 2 ‖ 饱食+营养为 0 ‖ 攻击属性 < 1）时，近战**只击退、不造成伤害**。
- **小刀 soft block 挖掘损耗 ÷4**——羊毛 / 作物 / 花 / 树苗 / 藤蔓 / 草蕨等由 ÷2 改为 **÷4（0.5）**。
- **修复：骷髅 / 蜘蛛变种数值大范围偏离**——长逝 12/6/0.29、长逝守卫 8 攻/0.29、骨领主 20/5/0.26；木蛛 6/1/0.8/毒 240、洞窟蛛 16/4/1.0/毒 480、黑寡妇 6/1/0.8/毒 960、恶魔蛛 18/5/1.0/毒 480 + 缓慢 V。**相位蛛机制改正**：由自创"满血瞬移 10 次"改回 R196 **闪避制**（初始 `rand(3)+2` 次，受击即瞬移脱离并**抵消该次伤害**，每 100 tick 回补 1 次）。
- **修复：挖掘调度门控**——`R196BlockDigger` 改为 R196 僵尸语义（非禁挖物品 且（聪明 ‖ 持工具））；`ZombieDigGoal` 增加"看不见目标则不挖"视线门。
- **修复：熔炉烧炼产出比率**——沙→砂岩（热度 1）/玻璃（热度 ≥ 2）**每次消耗 4 个沙**、沙不足 4 不可烧；黏土→砖额外最多 **+3**。
...
- **远古骨领主 24/8/0.27**、**复仇僵尸每区块 1 上限**：原实现本就符合 R196。
- **等级加成闸门**：R196 条件为 `等级修正 < 0 ‖ 技能未启用 ‖ 拥有 FIGHTING 技能`；ICPM 永久废除技能系统 ≡ 技能未启用 ⇒ **无条件生效本就是忠实实现**。
...
## 1.1.1（2026-09-09 ~ 09-11）· 世界恶意机制重做 + 配置界面改版 + R196 逐值审计纠偏 + 火花/枯死作物忠实移植 + 贴图去相似化

> 版本号未提升，以下 09-10 ~ 09-11 的改动**直接计入 1.1.1**（本栏目内按时间倒序排列）。

本轮做了一次"逐系统回到 R196 反编译源码对着数值核"的全面审计（产出 `完成度对比-MITE-R196-v6/v7.md`），把此前凭印象写下的机制全部拉回源码真值，并把缺失的忠实机制补齐。

### 火焰与燧石（R196 原文纠偏）
- **推翻自创"燃烧计数"机制**——R196 源码全局不存在 `ignition`/`combust` 任何痕迹，"点燃计数 / 前 4 次短燃 3 tick / 满 8 次烧毁"整套属凭空捏造，且会拦掉营火/蜡烛/TNT 与普通放火。已整体删除（`CombustionHandler`、`FireBlockMixin`、死标签 `icpm:combustible`/`icpm:plant`）。
- **忠实移植 R196 火光花**——新增 `icpm:spark`（`BlockSpark extends BlockFire`）：燧石打火实际放置火花方块，**2 tick 后**邻格可燃（或下方地狱岩）则变火、否则消失；**烧不起来也照扣 1 点耐久**（R196 燧石 maxDamage 仅 16）；火花不可获得、无 BlockItem、不参与蔓延。
- 火消耗可燃方块、蔓延概率表沿用 vanilla（与 R196 一致），火依旧是 MITE 里最危险的东西。

### 饱食度系统（按 `FoodStats.java` 重构）
- **修复：饥饿乘数被简化**——旧版只有"营养不良 ×1.5"。现按 R196 `getWetnessAndMalnourishmentHungerMultiplier` 完整实现：`1 + 潮湿 + (营养不良 ? 0.5 : 0)`；潮湿取淋雨（雷暴 0.5 / 普通 0.25）与浸水（头格 0.5 / 仅脚格 0.25）的最大值，寒冷群系 ×2、炎热群系归零——**淋雨、泡水、进雪原都会实打实加快消耗**。
- **移除"睡觉 ×20 饥饿"**——R196 该分支条件是 `inBed() && isOnHitList()`，而 `isOnHitList()` 只匹配作者硬编码的开发者账号，正常游玩永不触发（属 R196 死代码），旧版"一睡觉就 20 倍消耗"是自创惩罚，已删。
- **修复：回血少了两项条件**——补上 `naturalRegeneration` 游戏规则判定与 `1 + 回血附魔等级 × 0.5`（`icpm:regeneration`，胸甲位）。

### 工具耐久与伤害（逐值对齐 `ItemTool` 系）
- **修复：6 类工具耐久衰减率与 R196 不符**——剑（挖方 2.0 / 打怪 0.5）、锄（2.0 / 2.0）、匕首（继承剑）、小刀（剑 ÷2，作物 ÷4）、棍棒（继承短棒 0.25）、鸭嘴锄（铲 ×0.8 = 0.4）；另补镰刀"割草 0.5"与"吸血 1.0 / 锋利 2.0"特例、斧头"砂岩 1.875"特例。旧值导致剑挖方块损耗减半、打怪损耗翻倍、棍棒损耗达 4 倍。
- **修复：皮革耐久系数被砍到 40%**——`0.4 / 附魔 15` → R196 真值 `1.0 / 10`（皮甲、皮工具耐久恢复正常）。
- **修复：工具攻击伤害为自创公式**——72 处工具注册参数统一改为 R196 口径（材质伤害由材质层承担，注册参数 = 工具基础伤害 − 1），铁战斧等一批工具面板伤害回到 R196 值。
- **修复：区域张力缺低难度分支**——补 R196 `difficulty < 2 → ×difficulty/2`（和平/简单难度张力正确减半）。

### 作物枯死（R196 `BlockCropsDead` 补全）
- **新增 `icpm:dead_crop` 枯死作物方块**：保留生长进度、无随机刻、骨粉无效、破坏零掉落，贴图用 MITE 风格的枯死小麦 7 阶段。
- **旱死**：耕地干燥（无水）时每随机刻 5% ——**未成熟**转枯死（保留进度）、**已成熟**按原版掉落表掉收获物后枯死；此前只掉种子且不生成枯死态。
- **疫病致死**：染病作物每随机刻 1/64 转为枯死（成熟的进度 −1）；疫病扩散不再把枯死作物当传染目标。

### 兼容与清理
- **修复：Fabric 端营养 HUD 在新版客户端必崩**——1.21.11 的 HUD 已改为分层系统，旧注入点 `Gui.renderHotbarAndDecorations` 被删除；改用 Fabric 官方 HUD 层 API 注册在经验条之上（NeoForge 端此前已用等效方案）。
- **修复：Fabric 端一处 mixin 嵌套类型隐患**（与 NeoForge 端曾崩的 `IllegalClassLoadError` 同源），已外移为普通类。
- **修复：金属修复材料缺"粒"**——标签目录写成了 1.21 不再加载的 `tags/items/`（正确为 `tags/item/`），导致修复材料表从未生效；现已合并为"锭 + 粒"并删除无效目录。
- 清理一批确认零引用的死代码（重复的旧回血管理器、与源码冲突的旧材质表、未注册的伪 mixin、死字段与无效标签、失效注释），避免后人误引用。

### 贴图去相似化（版权友好化）
- 全量比对发现 **496 张贴图与 MITE 资源包逐像素完全相同**；现对它们做了**保留风格的调色板级微调**（色相 ±10°、饱和度/明度微调、每调色板色 ±2 通道，透明度与像素形状完全不变）——**现在 0 张与原作逐像素相同**，肉眼观感与原有风格一致。自绘贴图未改动。

### 关于"技能/专精"系统
- 经源码考据确认：R196 的技能（专精）系统属**作者未完成的废稿**（无成长途径、仅调试命令可授予，开启后无法正常游玩）。故**明确不实现**，特此说明以免误解为遗漏。

### 1.1.1 首版内容（2026-09-09）

#### 配置界面（malilib，按 H 打开）
- **改版为标签页 UI**：废除"疯狂劲敌/实验性玩法/参数配置/杂项"，仅保留 **世界恶意** 与 **上天眷顾** 两标签（当前标签灰置、另一标签可点击切换）；右侧 ⟲ 重置 / 默认顺序按钮（占位）。
- **修复：配置界面打开即崩溃 ×3 轮**——①列表项须用 `ConfigOptionWrapper.createFor` 包装（直接返回 ConfigBoolean 会 ClassCastException）；②标签/按钮 hover 数组传 `null` 致 NPE（改传 `new String[0]`）；③反射打开层日志补全堆栈便于定位。
- **真正可打开的路径**：注册进 malilib 的 mod 配置屏注册表（`Registry.CONFIG_SCREEN`，反射降级，兼容官方 masa 版）；"ICPM 配置"键默认绑定 H 并补中英文翻译。
- **上天眷顾**：启用创造模式（enableCreativeMode）——`config/icpm.json` 开关，false 时拒绝切换创造（`/icpmconfig` 命令与直接改 JSON 同样生效）。

#### 世界恶意（三个全新机制开关）
- **女巫低吟（witchWhisper）**：开启即被**永久诅咒**（独立 `witch_whisper` 效果位，HUD 可见、随原版效果栈持久化）——去咒药水无法解除/变更类型；不占普通诅咒槽，女巫仍可再咒你（双效果并存）。
- **噩梦时代（nightmareEra）**：主世界**永夜**（每日到清晨快进回黄昏，天数照常走）；每天 **80% 概率血月**（确定性伪随机，同一天恒定）。
- **技术不佳（poorTechnique，0~4 档）**：每档 耐久消耗 +25%、攻击力 -25%、挖掘速度 -25%——**忠实叠加在 ICPM/R196 运算之上**（腐蚀诅咒 ×2 与档位相乘、最终扣损点统一层叠，无任何平衡裁剪）。

#### R196 完成度收口（v5 剩余项）
- **A4 屠宰全量对齐**：羊掉落按 R196 全量实现（未剪未燃 50% 羊毛按色 + 健康羊掉 `rand(2)+rand(1+屠宰)` 羊肉 + 50% 皮革）；牛皮革纠为 `1..3 恒掉`；猪去掉误加的 50%+1。
- **全套怪物索敌**：索敌半径统一按 R196 钳制 = `min(followRange, 24)`、受伤 100 tick 内 ×2（`TargetGoal.getFollowDistance` 单点注入，覆盖全部 goal 系怪物；潜行/隐身/视线修正由 1.21.11 原生机制等价提供）。

> 说明：以上全部改动均已双端构建通过、服务端冒烟 `Done (0.4s)!`、mixin 注入点审计（142 个 mixin / 227 个注入点）全绿后部署；产物为 `ICPM-1.1.1.jar` / `ICPM-1.1.1-fabric.jar`。
...
- **摔落半血动物 10% 掉肉**：被动生物（CREATURE）受摔落伤害 ≥ 半血致死 → 仅 10% 概率掉落（FallHalfDropMixin）。
- **摔落易碎缓冲**：摔在玻璃/玻璃板/雪块上最大 5 伤且方块被砸碎（FragileLandingMixin）。
- **火焰/爆炸保护核查**：原版 1.21.11 fire_protection 已含「燃烧时间 −15%/级」，与 R196 一致无需改；爆炸保护 −15%/级与 1.21 的 protection 点数模型冲突，暂记取舍。

- **打草固定 0.2 掉种子**（BlockTallGrass 移植，短草/高草/蕨破坏时 20% 出 1 种子，不吃收获附魔）。
- **驯狼概率 R196 化**（EntityWolf.getTamingOutcome）：直接失败并攻击 5% / 无效果 5% / 直接成功 10% / 其余 80% 按经验加成（roll += rand×等级×0.02，≥0.8 成功）；失败后 100 tick 冷却；蓝月夜失败不攻击。

- **屠宰附魔修正**（R196 EnchantmentButchering）：鸡/兔不再受屠宰加成（R196 鸡豁免）；杀蜘蛛按公式掉蜘蛛眼 1−2/(3(n+1))；动物附加肉量改 rand(0..n) 并火烧致死出熟肉（基础 1+(0..2) 仍由原版掉落提供，完整 R196 总量需抑制 vanilla 肉类池，取舍待定）。
- **枯灌木 5% 掉木棍**（sky：灌木掉落木棍概率 5%）。

- **农业校准（种植大全）**：
  - 新增 ICPMClimate 群系温湿度表（R196 表：雪原0/0.5…地狱2.0/0，未收录回退原版基础温度），提供温度/湿度/群系生长影响(1−|t−[0.8,1.2]|)/生病因子(1−|t−[1.0,1.2]|)。
  - 干湿规则：下方耕地未湿润 → 95% 跳过随机刻、成熟作物干旱死亡掉落种子；必须保持耕地湿润。
  - 生病概率 R196 化：0.0005/t（土豆 0.001）× 温度疾病因子 ×（湿度>0.85?1.5）×（1−亮度/16），替代固定 1/512。
  - 行植与围困：同作物直线相邻（东/西或南/北单侧）每格 ×(1+0.5) 最多两层；四向或斜向被围困 ×0.5。
  - 群系温度影响并入生长速度（乘算）；肥力/丰收月/季节加成保留。

- **史莱姆/果冻腐蚀（R196 完整公式）**：类型倍率 M=绿史莱姆1/黄果冻2/红团块3/灰软泥3/黑布丁4。
  - 近战打史莱姆：腐蚀手持物，每次扣 100×M 耐久。
...
- **惧狼驯服（R196 EntityDireWolf）**：野狼默认不追杀玩家（改掉旧“蓝月索敌”倒置——R196 蓝月夜安静）；仅 4 格内 0.4%/tick 随机扑咬；喂骨驯服概率 20% 失败攻击/20% 无效/5% 直接成/其余 roll+=rand×等级×0.02 分档（<0.5 攻、<1.0 无效、≥1.0 成）；失败 100t 冷却、蓝月夜失败不攻击；驯服后 24 血、跟随主人/御敌/空手右键坐起、不阻止睡觉、状态 NBT 持久化。
- **马驯服逆反（R196 sky 差值）**：健康野马被喂食后进入 4000 tick 逆反窗口——期间再喂食后腿直立怒声拒绝且不消耗食物（治疗受伤野马不触发）；野马被玩家击伤 temper −10。
- **核查通过项**：负等级惩罚（-2% 挖掘/合成+近战按 R196 getLevelModifier 已一致）；拥挤生病（isCrowded 5×5/>2 只/室内 与 R196 逐字同构）；守卫切换（LongdeadGuardian >6 弓 / <5 斧已一致）。

- **玩家死亡掉落存活 15 分钟**（R196 EntityPlayer 掉落物 age=-18000 语义移植）：Player.dropAllDeathLoot TAIL 将死亡瞬间刚生成的掉落物 age−12000（+原版 6000 上限≈18000 tick=15 min）；平时丢出的物品仍 5 分钟。
- **银甲类型化减伤（R196）**：移除旧的「亡灵攻击 5%/件 减伤」（非 R196）；新增毒时长抗性=银覆盖率×0.5（头0.2/胸0.4/腿0.3/靴0.1），中毒持续时间按 (1−resist) 缩放（R196 getResistanceToPoison）；银武器 vs 亡灵 +2.5 Smite 保留。Drain/Shadow 同公式但 ICPM 暂无伤害源；麻痹抗性 R196 归「自由行动」附魔不属银甲。

- **A4 屠宰全量对齐（牛/猪 R196 dropFewItems 精确移植）**：牛=皮革 0..2 恒掉 + **仅健康(isWell)出肉** rand(3)+1+rand(1+屠宰级)、总 1 时 50%+1、燃烧→熟；猪=仅健康出肉同公式。**不健康动物在 R196 不掉肉**——取代 vanilla 常掉 1..3 肉（此前的全量缺口）。牛/猪从屠宰 mixin 剔除防叠加；羊/鸡/兔保持近似（鸡兔豁免按 R196）。
- **TNT 禁炸项（ITF 实测结论落地）**：艾德曼矿石系(含深板岩)与金属块(silver/ancient_metal/mithril_block)爆炸抗性→3600000（可挖不可炸）；艾德曼块原 1200 类黑曜石保留；其余矿石/方块按原版（用户 1.6.4-ITF 实测"除艾德曼矿系+金属块禁炸外皆谣传"）。

- **生存获取修复（洋葱）**：主世界刷怪笼(地牢)箱子 25% 概率 1-2 洋葱（simple_dungeon.json 新池）——此前洋葱仅创造/配方存在、生存无来源。蓝莓：R196 为可交互"蓝莓丛"(BlockBush) 森林生成；ICPM 仅有裸物品、缺方块+世界生成，列入下一轮专项。
...
- **盔甲穿戴贴图错乱**：4 套板甲（银/远古金属/秘银/艾德曼）的 `textures/models/armor/*_layer_1/2.png` 从未入库 → 穿戴紫黑缺失纹理；已从 MITE Resource Pack 1.6.41 补齐并全量同步各材质层贴图（白底校验通过）。
- **触碰距离分通道 v2（R196 getReach 忠实化）**：方块 2.75 / 实体交互 2.5 / 近战攻击 1.5+高度优势+武器加成，攻击距离在 `Player.attack` HEAD 按 AABB 裁决，不再误钳实体交互通道。
- **启动崩溃收口**：`@Mixin(子类)` 注入仅父类声明方法（appendHoverText / die）→ 一律改 @Mixin 声明类 + instanceof 守卫；`@ModifyVariable` argsOnly 签名含重复被改参。
- 产物文件名改纯 ASCII `ICPM-*.jar`（CurseForge 拒绝含中文文件名）。
- **石桶对齐 R196**：石桶不再能右键放置（`contains(stone)→return false`）；取消"桶+圆石→石桶"合成（非 R196 内容），新增 R196 拆解配方（石桶→对应空桶）；岩浆桶遇水冷却→石桶（玩家浸水背包 + 掉落物入水，嘶嘶蒸汽）。
- **饱食系统对齐修正**：饥饿伤害触发条件改为 R196 `isStarving`=satiation==0（原误绑 nutrition==0）；睡眠回血倍率 4.0（原 8.0）；禁合成/减速等"全惩罚"经源码核对为讹传（R196 无该分支）。

## 1.0.6

**新增：R196 水源·桶机制（接取不耗源头 / 放流动 / Ctrl+100XP 放源头 / 熔化概率）· 2026-09-03**

㊳ R196 水源·桶机制移植（ItemBucket + ItemVessel 判决）：

- **接取不耗源头**：空桶（原版铁桶与 ICPM 铜/银/金/远古金属/秘银/艾德曼桶）右键水/岩浆源头——生存不再删除液体块（源头保留），桶正常变为对应满桶；创造才消耗源。
- **接岩浆熔化概率**（shift 查看 tooltip，仅岩浆桶变体显示）：艾德曼 0%、金 20%、铜/银 16%、铁 8%、远古金属 4%、秘银 1%（R196 `0.01×秘银耐久÷材质耐久`）；熔化时整桶被岩浆吞没（仅艾德曼免疫，is_harmed_by_lava）。
- **放置只放"流动"**：生存普通右键倒水/岩浆 → 放流动液块；密闭无出路时 1 秒内结晶为源头（等效 R196 moving→still schedule）；往同种液体上倒会被取消（桶变空不放置，防倒海造源）。
- **Ctrl+右键 消耗 100 经验放置源头**（水/岩浆按桶内容物；创造免费）——客户端按键检测 + C2S 包（bucket_source）服务端权威执行并同步手持空桶。
- 涉及：ICPMBucketRules / BucketItemR196Mixin（原版三件套）/ ICPMBucketItem.use 委托 / BucketSourcePacket / BucketCtrlClientMixin / BucketTooltipMixin / lang 5 键。
...
㊱ 砧去附魔（R196 ContainerRepair is_disenchanting · 2026-09-03）：

- ICPM 金属砧（MetalAnvilMenu）材料槽放行 `icpm:bottle_of_disenchanting`；
- 预览：装备（带附魔）+ 去咒药水 → 结果 = 装备副本移除附魔（保留其它 NBT/命名）；无附魔则提示并置空；
- 取件：消耗 1 瓶去咒药水 + 消耗原装备，不耗金属粒、不损耗砧。

㊲ 胰岛素抵抗 / 糖尿病（R196 判决移植 · 2026-09-03）：

- **机制（R196 源码判决）**：胰岛素抵抗 = 玩家累计值 int（0..192000），进食高糖食物累加（反应 = 糖值 × 4.8），每 tick 自然代谢 −1；抵抗值达到 **48000/96000/144000** 进入轻度/中度/重度（阈值非 100%，为慢性累积型）。
- **等级平移**（setInsulinResistance）：归零即痊愈；抵抗值未归零时跌回轻度阈值以下仍保留轻度（得病不轻易自愈）；升级直升，降档按单步回落。
- **副作用**：胰岛素抵抗期内每次进食高糖 → 立即恶心 400t（强度=等级）；重度额外中毒 duration=max(本次糖反应/48, 100)；中度以上"不能代谢糖" → 饱食收益按糖值/1000 打折。
- **接入**：`ICPMInsulinResistance`（服务端表 + 玩家 NBT `IcpmInsulinResistance`/`IcpmInsulinLevel`）+ `InsulinFoods` 糖含量表（原版糖/蜂蜜/曲奇/蛋糕/水果/奶 + ICPM 巧克力/冰淇淋/橙/香蕉/蓝莓/麦片/牛奶碗/粥等），挂进食统一漏斗 FoodNutritionMixin + 每 tick 代谢。
- 说明：R196 恶心效果 = 1.21 的 Nausea；持久化走玩家 NBT（与 R196 同键语义）。

㊳ 火焰烧肉重构（R196 源码判决 · 2026-09-03）：

- **清除混元3 自创状态机**：删除"点一次火烧 1/4（COOK_UNIT=25）、须离火再放回才继续、点 4 次熟"的窗口模型、"夹生保火"（hasRawNearby 不灭）、熟食永久防火、以及 campfire/熔炉/烟熏炉"站旁边即烤"的错误热源。
- **还原 R196 判决语义**：生食掉落物持续受火焰灼烧即连续累计 `cooking_progress += 火伤×3`，满 100 转成熟食（数量不变）+ 滋滋声 + 按熟食经验掉落；烤熟瞬间给周围 3×3 火排程 10 tick 后执行堆料灭火（该火格可烤生食堆叠 <2 不灭、≥2 以 0.01×2^n 概率灭、>15 强制灭）；熔岩同属火焰伤害（掉落物在岩浆里被烤熟而非销毁），热源 tag 收窄为 fire/soul_fire/lava。
- 现代 MC 适配仅保留：受热生食/刚熟熟食临时防火（等效 R196"受伤不死的 health 体系"）。
